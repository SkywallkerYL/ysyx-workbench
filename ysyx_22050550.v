...
//Clint 
assign Flush = iruflush || fence || Dcache_fence      ; 
wire clintinterrupt ;
wire ClintRen  ;
...
//wire		Icache_w_ready	;
wire		Icache_aw_valid = 0 ;
wire [63:0] Icache_aw_addr	= 0;
wire [7:0]  Icache_aw_len	= 0 ;
wire [2:0]  Icache_aw_size	 = 0;
wire [1:0]	Icache_aw_burst  = 0; 
wire		Icache_w_valid   = 0; 
wire [63:0]	Icache_w_data	 = 0;
wire [ 7:0] Icache_w_strb	= 0;
wire		Icache_w_last	 = 0;
//wire		Icache_b_ready	;
wire		Icache_busy		; 
ysyx_22050550_ICACHE ICache(
    .clock        (clock)             ,
    .reset        (reset)             ,
        ...
        case (state)
            ...
            refill:begin
				...
				if(io_r_valid && io_r_ready)begin
                    if (io_r_last && io_r_rresp == 2'b00) begin
						if(Reglen == 2'd3)
							next = lookup;
						else next = miss; 
                    end
...
`endif 
    ...
    //cache 状态机 idle :: lookup :: miss :: replace :: refill 
    localparam idle = 3'd0, lookup = 3'd1, miss = 3'd2, refill = 3'd4;
	//localparam wresp = 3'd5 ;// fence = 3'd6 ; //another state to handle bresp 
	//添加fence.i指令的实现     
	//将cache 内的dirty line 给全部写回 ，然后将多有的cacheline 给置无效。。
	//cache  内置一个寄存器  ，只有在idle 或者能跳回idle的态，才能跳转到fencei
    ...
    */
    ...
    //Tag的写使能已经在tag初始化的时候处理了
    //Data 只有这两种情况需要写入
    assign DataWen = !((REFILL&&(io_r_valid)));
    assign DataWrite = (REFILL&&(io_r_valid)) ? refilldata :0;
    assign DataBen   = (REFILL&&(io_r_valid)) ? refillben  : {128{1'b1}};
    //Data 需要读出的情况有 一：IDLE并且valid 二：refill完成跳转回lookup
    //三向总线写回cacheline miss的时候要跳replace 或者在replace并且不会跳miss
    //这样子该后可以在部分周期内省掉对寄存器的操作，仿真更快
    assign DataCen = 1'b0;
