...
)
{
	...
	uint8_t *data = NULL;
   //printf("TIL_OpenRequiredFile\n");
	int data_size = 0, dos_score = 0, win_score = 0;
	BOOL result = FALSE;
   //nanoslite用menu运行到这里会卡死
	for (int i = 0; i < sizeof(fps) / sizeof(FILE *); i++)
	{
      //printf("file i:%d ind%d\n",i,sizeof(fps) / sizeof(FILE *));
		//
...
--*/
{
   ...
   //printf("Detect game language only when no message file specified\n");
   //
   // Set decompress function
   //
   Decompress = gConfig.fIsWIN95 ? YJ2_Decompress : YJ1_Decompress;
   //printf("Set decompress function\n");
