...
class RamArbiter extends Module{
    ...
    io.ifu.Axi.ar.ready := io.ifu.Axi.ar.valid && io.sram.Axi.ar.ready
    io.lsu.Axi.ar.ready := io.lsu.Axi.ar.valid && io.sram.Axi.ar.ready &&(!io.ifu.Axi.ar.valid)
    io.sram.Axi.ar.bits.addr := Mux(io.ifu.Axi.ar.ready,io.ifu.Axi.ar.bits.addr,
    Mux(io.lsu.Axi.ar.ready,io.lsu.Axi.ar.bits.addr,0.U))
    //r
    io.sram.Axi.r.ready := io.ifu.Axi.r.ready || io.lsu.Axi.r.ready
    io.ifu.Axi.r.valid := io.ifu.Axi.r.ready && io.sram.Axi.r.valid
    ...
    io.ifu.Axi.aw.ready := io.ifu.Axi.aw.valid && io.sram.Axi.aw.ready
    io.lsu.Axi.aw.ready := io.lsu.Axi.aw.valid && io.sram.Axi.aw.ready &&(!io.ifu.Axi.aw.valid)
    io.sram.Axi.aw.bits.addr := Mux(io.ifu.Axi.aw.ready,io.ifu.Axi.aw.bits.addr,
    Mux(io.lsu.Axi.aw.ready,io.lsu.Axi.aw.bits.addr,0.U))

    //w
    io.sram.Axi.w.valid := io.ifu.Axi.w.valid || io.lsu.Axi.w.valid
    io.ifu.Axi.w.ready := io.ifu.Axi.w.valid && io.sram.Axi.w.ready
    io.lsu.Axi.w.ready := io.lsu.Axi.w.valid && io.sram.Axi.w.ready &&(!io.ifu.Axi.w.valid)
    io.sram.Axi.w.bits.addr := Mux(io.ifu.Axi.w.ready,io.ifu.Axi.w.bits.addr,
    Mux(io.lsu.Axi.w.ready,io.lsu.Axi.w.bits.addr,0.U))
    //b
    io.sram.Axi.b.ready := io.ifu.Axi.b.ready || io.lsu.Axi.b.ready
    io.ifu.Axi.b.valid := io.ifu.Axi.b.ready && io.sram.Axi.b.valid
    io.lsu.Axi.b.valid := io.lsu.Axi.b.ready && io.sram.Axi.b.valid &&(!io.ifu.Axi.b.ready)
