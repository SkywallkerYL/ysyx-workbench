...
//R
  ...
  //slt 目前有问题
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt   , RI, R(dest) = src1<src2);
...
//bge src1 与src2 均视为二进制补码
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, int64_t rs1 = src1;int64_t rs2 = src2;imm = SEXT(imm,12);s->dnpc = (rs1>=rs2)?s->pc+imm:s->pc+0x4);
...
//但是这样转化后，blt slt 可以正常运行，可能还没碰到极端情况
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, int64_t rs1 = src1;int64_t rs2 = src2;imm = SEXT(imm,12);s->dnpc = ((rs1)<rs2)?s->pc+imm:s->pc+0x4);
//S
