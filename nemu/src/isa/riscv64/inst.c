...
//s否则会识别为srai shamt bits[25:20]
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, imm = SEXT(imm,12);word_t val = Mr(src1 + imm, 1);val = SEXT(val,8);R(dest)=val);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, imm = SEXT(imm,12);word_t temp = Mr(src1+imm,2);R(dest) =SEXT(temp,16));
...
//R
  ...
  INSTPAT("0100000 ????? ????? 000 ????? 01110 11", subw  , RI, word_t val = src1-src2;CUT(val,32);val = SEXT(val,32);R(dest) = val); 
