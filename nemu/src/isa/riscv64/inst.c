...
//R
  ...
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", slt   , RI, R(dest) = src1<src2 );
...
//if-else 程序31步 判断s2 与a5的大小可以看出来  这里判断的时候还是按照无符号数来的
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, int64_t rs1 = src1; int64_t rs2 = src2;s->dnpc = (rs1<rs2)?s->pc+imm:s->pc+0x4);
//S
