...
//s否则会识别为srai shamt bits[25:20]
  ...
  //移位指令目前实现有问题
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, word_t shamt=SEXT(imm,12);R(dest) = src1 << shamt);
  INSTPAT("000000? ????? ????? 001 ????? 00110 11", slliw   , I, word_t shamt=SEXT(imm,12);R(dest) = SEXT((src1 << shamt),32));
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, word_t shamt=SEXT(imm,12);R(dest) = src1 >> shamt);
...
//但是这样转化后，blt slt 可以正常运行，可能还没碰到极端情况
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, imm = SEXT(imm,12);s->dnpc = ((src1)<src2)?s->pc+imm:s->pc+0x4);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, imm = SEXT(imm,12);s->dnpc = ((src1)>=src2)?s->pc+imm:s->pc+0x4);
...
//S
  ...
  INSTPAT("??????? ????? ????? 011 ????? 01001 11", fsd    , S, imm = SEXT(imm,12);Mw(src1 + imm, 8, src2));
