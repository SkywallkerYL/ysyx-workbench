...
//s否则会识别为srai shamt bits[25:20]
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, word_t shamt=SEXT(imm,6) ;R(dest) = src1 >> shamt);//  
  ...
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw  , I, word_t shamt=SEXT(imm,6);R(dest) =  SEXT((src1 >> shamt),32));
