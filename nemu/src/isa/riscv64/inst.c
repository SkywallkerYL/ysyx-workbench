...
//s否则会识别为srai shamt bits[25:20]
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, word_t val = src1+(imm);R(dest) = (val<<32)>>32);//结果截断为32位
...
//R
  ...
  //slt 目前有问题
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt   , RI, int rs1 = src1;int rs2 = src2;/*printf("src1:%ld src2:%ld\n",src1,src2);*/R(dest) = rs1<rs2);
