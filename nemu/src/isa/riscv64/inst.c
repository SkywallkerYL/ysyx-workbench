...
//s否则会识别为srai shamt bits[25:20]
  ...
  //移位指令目前实现有问题
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, word_t shamt = (imm<<52)>>52;R(dest) = src1 << shamt);
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, word_t shamt = (imm<<52)>>52;R(dest) = src1 >> shamt);
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, u_int32_t val = src1+imm;R(dest) = val);//结果截断为32位
...
//seqz 被扩展为 src1<1  等于0置位
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, /*unsigned short immu = imm;*/R(dest) = (src1<imm));
...
//R
  ...
  //slt 目前有问题
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt   , RI, /*int64_t rs1 = src1;int64_t rs2 = src2;*/printf("src1:%ld src2:%ld\n",src1,src2);R(dest) = (src1-src2)<0 );
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw  , RI, R(dest) = src1*src2;R(dest)= (R(dest)<<32)>>32);//截断为32
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw  , RI, src1=(src1<<32)>>32;src2=(src2<<32)>>32;R(dest) = src1/src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw  , RI, src1=(src1<<32)>>32;src2=(src2<<32)>>32;R(dest) = src1%src2);
//B
  //beqz 是=0分支    src2 = 0
...
//bge src1 与src2 均视为二进制补码
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = ((src1-src2)>=0)?s->pc+imm:s->pc+0x4);
// blt 有问题，因该是比较时视为二进制补码没有实现好
//if-else 程序31步 判断s2 与a5的大小可以看出来  这里判断的时候还是按照无符号数来的 但是无符号数的加减法等同于
//应该作加减与0比较，不要直接比较大小，这样可以避免一个整数 小于 -1的补码（全1）的情形
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, int64_t rs1 = src1;int64_t rs2 = src2;printf("rs1:%ld rs2:%ld src1:%lu src2:%lu",rs1,rs2,src1,src2);s->dnpc = ((rs1)<rs2)?s->pc+imm:s->pc+0x4);
//S
