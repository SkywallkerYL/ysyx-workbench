...
//R
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw  , RI, int32_t  rs1 = low32(src1);src2 = SEXTU(src2,6);R(dest) = SEXT((rs1>>src2),32));
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw  , RI, uint32_t rs1 = low32(src1);src2 = SEXTU(src2,6);R(dest) = SEXT((rs1>>src2),32));
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl   , RI, uint32_t rs1 = low32(src1);src2 = SEXTU(src2,6);R(dest) = (rs1>>src2));
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem   , RI, int64_t val=(int64_t)src1%(int64_t)src2;R(dest) = val);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu  , RI, word_t  val=src1%src2;R(dest) = val);
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw  , RI, src1 = SEXTU(src1,32);src2 = SEXTU(src2,32);int32_t rs1 = src1;int32_t rs2 = src2;int32_t val=rs1%rs2;R(dest) = SEXT(val,32));
  INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw  , RI, src1 = SEXTU(src1,32);src2 = SEXTU(src2,32);word_t rs1 = src1;word_t rs2 = src2;int32_t val=rs1%rs2;R(dest) = SEXT(val,32));
...
//S
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));cpu.rwaddr=src1 + imm;cpu.wdata=(uint16_t)src2;
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));cpu.rwaddr=src1 + imm;cpu.wdata=(uint8_t) src2;
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));cpu.rwaddr=src1 + imm;cpu.wdata=(uint32_t)src2;
  INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd     , S, Mw(src1 + imm, 8, src2));cpu.rwaddr=src1 + imm;cpu.wdata=(uint64_t)src2;
