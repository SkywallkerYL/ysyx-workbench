...
# 124 "src/isa/riscv64/inst.c"
  ...
  do { uint64_t key, mask, shift; pattern_decode("1111000 00000 ????? 000 ????? 10100 11", (sizeof("1111000 00000 ????? 000 ????? 10100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); src1 = ({ struct { int64_t n : 32; } __x = { .n = src1 }; (uint64_t)__x.n; });(cpu.gpr[check_reg_idx(dest)]) = ({ struct { int64_t n : 32; } __x = { .n = src1 }; (uint64_t)__x.n; }) ; }; goto *(__instpat_end); } } while (0);
