...
static int decode_exec(Decode *s) {
  ...
  do { uint64_t key, mask, shift; pattern_decode("0000000 00000 00000 000 00000 11100 11", (sizeof("0000000 00000 00000 000 00000 11100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); s->dnpc = isa_raise_intr((cpu.gpr[check_reg_idx(17)]),cpu.pc) ; }; goto *(__instpat_end); } } while (0);
...
# 174 "src/isa/riscv64/inst.c"
  ...
  do { uint64_t key, mask, shift; pattern_decode("0011000 00010 00000 000 00000 11100 11", (sizeof("0011000 00010 00000 000 00000 11100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); s->dnpc = mret_inst();printf("dnpc:%lx\n",s->dnpc) ; }; goto *(__instpat_end); } } while (0);
