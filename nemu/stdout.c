...
# 48 "src/isa/riscv64/inst.c" 3 4
                            _Bool 
# 48 "src/isa/riscv64/inst.c"
...
void csrrs (word_t csr, word_t src1,word_t rd) {
  ...
  switch (csr)
  {
  case 0x305: t = cpu.mtvec;cpu.mtvec = t|src1;(cpu.gpr[check_reg_idx(rd)]) = t;
    break;
...
static int decode_exec(Decode *s) {
  ...
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 010 ????? 11100 11", (sizeof("??????? ????? ????? 010 ????? 11100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); csrrs(imm,src1,dest) ; }; goto *(__instpat_end); } } while (0);
