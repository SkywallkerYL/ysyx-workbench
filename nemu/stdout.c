...
# 47 "src/memory/paddr.c"
 "0"
# 47 "src/memory/paddr.c" 3 4
 , "src/memory/paddr.c", 47, __extension__ __PRETTY_FUNCTION__); }))
# 47 "src/memory/paddr.c"
 ; } } while (0)
                                          ;
}
...
# 63 "src/memory/paddr.c" 3 4
 "x" 
# 63 "src/memory/paddr.c"
 "]" "\33[0m" "\n", "src/memory/paddr.c", 63, __func__, ((paddr_t)0x80000000), ((paddr_t)0x80000000 + 0x8000000 - 1)); do { extern FILE* log_fp; extern 
# 63 "src/memory/paddr.c" 3 4
 _Bool 
# 63 "src/memory/paddr.c"
...
# 63 "src/memory/paddr.c" 3 4
 "x" 
# 63 "src/memory/paddr.c"
 "]" "\33[0m" "\n", "src/memory/paddr.c", 63, __func__, ((paddr_t)0x80000000), ((paddr_t)0x80000000 + 0x8000000 - 1)); fflush(log_fp); } } while (0); } while (0);
}
# 470 "src/memory/paddr.c"
word_t paddr_read(paddr_t addr, int len) {
...
void paddr_write(paddr_t addr, int len, word_t data) {
  if (__builtin_expect(in_pmem(addr), 1)) {
    ...
    cache_write(addr-0x80000000,data,len);
...
# 550 "src/memory/paddr.c" 2
# 560 "src/memory/paddr.c"

# 560 "src/memory/paddr.c"
void mem_read(uintptr_t block_num, uint8_t *buf){
  memcpy(buf, cache_pmem + (block_num << 6), (1 << (6)));
}
void mem_write(uintptr_t block_num, const uint8_t *buf){
  memcpy(cache_pmem + (block_num << 6), buf, (1 << (6)));
}

static uint64_t cycle_cnt = 0;

void cycle_increase(int n) { cycle_cnt += n; }
# 579 "src/memory/paddr.c"
struct CACHE
{
  
# 581 "src/memory/paddr.c" 3 4
 _Bool 
# 581 "src/memory/paddr.c"
      valid ;
  
# 582 "src/memory/paddr.c" 3 4
 _Bool 
# 582 "src/memory/paddr.c"
      dirty ;
  uintptr_t tag;
  uint8_t data[(1 << (6))];
};
...
static void writelenBytes(struct CACHE* temp,intptr_t addr,word_t data,int len){
  ...
  case 8 : *(uint64_t *) cache_data = data;return ;
    break;
  default: 
# 645 "src/memory/paddr.c" 3 4
          ((void) sizeof ((
# 645 "src/memory/paddr.c"
          0
# 645 "src/memory/paddr.c" 3 4
          ) ? 1 : 0), __extension__ ({ if (
# 645 "src/memory/paddr.c"
...
# 645 "src/memory/paddr.c"
          "0"
# 645 "src/memory/paddr.c" 3 4
          , "src/memory/paddr.c", 645, __extension__ __PRETTY_FUNCTION__); }))
# 645 "src/memory/paddr.c"
                   ;
    break;
  }
...
word_t cache_read(uintptr_t addr,int len) {
  ...
  for (int i = group_base; i < group_base+(1 << (assoc_width)); i++)
  {
  ...
  int line = ramdchoose((1 << (assoc_width)));
...
# 762 "src/memory/paddr.c"
void init_cache(int total_size_width, int associativity_width) {

  size_width = total_size_width;
  assoc_width = associativity_width;
  line_width = total_size_width - 6;
  group_width = total_size_width - 6 - associativity_width;
  block_mask = ((1 << (6)) - 1);
  group_mask = ((1 << (group_width)) - 1) << 6;
  tag_mask = ~((1 << (group_width+6)) - 1);

  cache = (struct CACHE *)malloc(sizeof(struct CACHE)*(1 << (total_size_width - 6)));

  
# 774 "src/memory/paddr.c" 3 4
 ((void) sizeof ((
# 774 "src/memory/paddr.c"
 cache
# 774 "src/memory/paddr.c" 3 4
 ) ? 1 : 0), __extension__ ({ if (
# 774 "src/memory/paddr.c"
...
# 774 "src/memory/paddr.c"
 "cache"
# 774 "src/memory/paddr.c" 3 4
 , "src/memory/paddr.c", 774, __extension__ __PRETTY_FUNCTION__); }))
# 774 "src/memory/paddr.c"
              ;
  for (size_t i = 0; i < (1 << (total_size_width - 6)); i++)
  {
    cache[i].valid = 0;
    cache[i].dirty = 0;
