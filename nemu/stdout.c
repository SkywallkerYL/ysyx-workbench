...
# 108 "src/isa/riscv64/inst.c"
  ...
  do { uint64_t key, mask, shift; pattern_decode("0000000 ????? ????? 010 ????? 01100 11", (sizeof("0000000 ????? ????? 010 ????? 01100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); printf("src1:%ld src2:%ld\n",src1,src2);(cpu.gpr[check_reg_idx(dest)]) = (src1-src2)<0 ; }; goto *(__instpat_end); } } while (0);
  ...
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 100 ????? 11000 11", (sizeof("??????? ????? ????? 100 ????? 11000 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_B); int64_t rs1 = src1;int64_t rs2 = src2;printf("rs1:%ld rs2:%ld src1:%lu src2:%lu",rs1,rs2,src1,src2);s->dnpc = ((rs1)<rs2)?s->pc+imm:s->pc+0x4 ; }; goto *(__instpat_end); } } while (0);
