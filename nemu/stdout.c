...
static int decode_exec(Decode *s) {
  ...
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 010 ????? 00100 11", (sizeof("??????? ????? ????? 010 ????? 00100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); printf("rs1:%ld imm:%ld\n cmp:%d",(int64_t)src1,imm,(int64_t)src1 < imm);(cpu.gpr[check_reg_idx(dest)]) = ((int64_t)src1 < imm) ; }; goto *(__instpat_end); } } while (0);
