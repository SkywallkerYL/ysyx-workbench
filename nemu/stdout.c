...
static int decode_exec(Decode *s) {
  ...
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????0 10? ????? ????? 01", (sizeof("??????? ????? ????0 10? ????? ????? 01") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); (cpu.gpr[check_reg_idx(dest)]) = imm ; }; goto *(__instpat_end); } } while (0);
