...
static void decode_operand(Decode *s, int *dest, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *dest = rd;
  ...
  switch (type) {
    ...
    case TYPE_J: do { *imm =(({ struct { int64_t n : 1; } __x = { .n = (((i) >> (31)) & ((1ull << ((31) - (31) + 1)) - 1)) }; (uint64_t)__x.n; })<<19)|(((i) >> (12)) & ((1ull << ((19) - (12) + 1)) - 1))|(((i) >> (20)) & ((1ull << ((20) - (20) + 1)) - 1))|(((i) >> (21)) & ((1ull << ((30) - (21) + 1)) - 1)); } while(0); break;
  }
  printf("%lx\n",*imm);
}
...
static int decode_exec(Decode *s) {
  ...
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 000 ????? 11001 11", (sizeof("??????? ????? ????? 000 ????? 11001 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); (cpu.gpr[check_reg_idx(dest)]) = s->pc+4;printf("%lx\n",s->pc);s->pc = (src1+imm)&~1 ; }; goto *(__instpat_end); } } while (0);printf("%lx\n",s->pc);


  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? ??? ????? 11011 11", (sizeof("??????? ????? ????? ??? ????? 11011 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_N); (cpu.gpr[check_reg_idx(dest)]) = s->pc+4;printf("%lx\n",s->pc);s->pc += imm;printf("%lx\n",s->pc) ; }; goto *(__instpat_end); } } while (0);
