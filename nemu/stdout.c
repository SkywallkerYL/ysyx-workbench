...
# 126 "src/isa/riscv64/inst.c"
  ...
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 011 ????? 01001 11", (sizeof("??????? ????? ????? 011 ????? 01001 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_S); imm = ({ struct { int64_t n : 12; } __x = { .n = imm }; (uint64_t)__x.n; });printf("rs1:%lx rs2:%lx imm:%lx\n",src1,src2,imm);vaddr_write(src1 + imm, 8, src2) ; }; goto *(__instpat_end); } } while (0);
