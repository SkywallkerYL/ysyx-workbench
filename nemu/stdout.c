...
# 54 "src/isa/riscv64/inst.c"
word_t isa_raise_intr(word_t NO, vaddr_t epc);
void log_ftrace(paddr_t addr,
# 55 "src/isa/riscv64/inst.c" 3 4
                            _Bool 
# 55 "src/isa/riscv64/inst.c"
...
# 74 "src/isa/riscv64/inst.c" 3 4
                                                 _Bool 
# 74 "src/isa/riscv64/inst.c"
                                                      z_imm,
# 74 "src/isa/riscv64/inst.c" 3 4
                                                            _Bool 
# 74 "src/isa/riscv64/inst.c"
  ...
  switch (csr)
  {
    case 0x305 : t = cpu.mtvec ;cpu.mtvec = read?(realsrc|t):realsrc; (cpu.gpr[check_reg_idx(rd)]) = t;break;
    case 0x300: t = cpu.mstatus;cpu.mstatus = read?(realsrc|t):realsrc;(cpu.gpr[check_reg_idx(rd)]) = t;break;
    case 0x341 : t = cpu.mepc ;cpu.mepc = read?(realsrc|t):realsrc; (cpu.gpr[check_reg_idx(rd)]) = t;break;
    case 0x342 : t = cpu.mcause ;cpu.mcause = read?(realsrc|t):realsrc; (cpu.gpr[check_reg_idx(rd)]) = t;break;
    case 0x304 : t = cpu.mie ;cpu.mie = read?(realsrc|t):realsrc; (cpu.gpr[check_reg_idx(rd)]) = t;break;
    default:
      break;
  }
}
...
word_t mret_inst(){
  ...
  if(cpu.mstatus&((1<<7))) cpu.mstatus = cpu.mstatus|((1<<3));
  else cpu.mstatus = cpu.mstatus&(~((1<<3)));
  cpu.mstatus = cpu.mstatus|(((1<<7)));
# 109 "src/isa/riscv64/inst.c"
  return cpu.mepc;
}
static int decode_exec(Decode *s) {
  ...
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 001 ????? 11100 11", (sizeof("??????? ????? ????? 001 ????? 11100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); csrrw_inst(imm,src1,s,0,0) ; }; goto *(__instpat_end); } } while (0);
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 010 ????? 11100 11", (sizeof("??????? ????? ????? 010 ????? 11100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); csrrw_inst(imm,src1,s,0,1) ; }; goto *(__instpat_end); } } while (0);
  do { uint64_t key, mask, shift; pattern_decode("0000000 00000 00000 000 00000 11100 11", (sizeof("0000000 00000 00000 000 00000 11100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); s->dnpc = isa_raise_intr((cpu.gpr[check_reg_idx(17)]),s->pc) ; }; goto *(__instpat_end); } } while (0);
  ...
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 010 ????? 00100 11", (sizeof("??????? ????? ????? 010 ????? 00100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); (cpu.gpr[check_reg_idx(dest)]) = ((int64_t)src1 < (int64_t)imm) ; }; goto *(__instpat_end); } } while (0);
  ...
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 010 ????? 00000 11", (sizeof("??????? ????? ????? 010 ????? 00000 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); word_t val = vaddr_read(src1+imm,4);val = ({ struct { int64_t n : 32; } __x = { .n = val }; (uint64_t)__x.n; });(cpu.gpr[check_reg_idx(dest)]) = val ; }; goto *(__instpat_end); } } while (0);
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 110 ????? 00000 11", (sizeof("??????? ????? ????? 110 ????? 00000 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_I); word_t val = vaddr_read(src1+imm,4);(cpu.gpr[check_reg_idx(dest)]) = (uint64_t)val ; }; goto *(__instpat_end); } } while (0);
...
# 186 "src/isa/riscv64/inst.c"
  ...
  do { uint64_t key, mask, shift; pattern_decode("0000000 ????? ????? 101 ????? 01100 11", (sizeof("0000000 ????? ????? 101 ????? 01100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); uint32_t rs1 = (src1&0x00000000FFFFFFFF);src2 = ({ struct { uint64_t n : 6; } __x = { .n = src2 }; (uint64_t)__x.n; });(cpu.gpr[check_reg_idx(dest)]) = (rs1>>src2) ; }; goto *(__instpat_end); } } while (0);
  ...
  do { uint64_t key, mask, shift; pattern_decode("0000001 ????? ????? 100 ????? 01100 11", (sizeof("0000001 ????? ????? 100 ????? 01100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); if(src2 == 0) (cpu.gpr[check_reg_idx(dest)])=0;else {word_t val = (int64_t)src1/(int64_t)src2;(cpu.gpr[check_reg_idx(dest)])= val;} ; }; goto *(__instpat_end); } } while (0);
  ...
  do { uint64_t key, mask, shift; pattern_decode("0000001 ????? ????? 101 ????? 01100 11", (sizeof("0000001 ????? ????? 101 ????? 01100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); word_t val=src1/src2;(cpu.gpr[check_reg_idx(dest)]) = val ; }; goto *(__instpat_end); } } while (0);
  do { uint64_t key, mask, shift; pattern_decode("0000001 ????? ????? 101 ????? 01110 11", (sizeof("0000001 ????? ????? 101 ????? 01110 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); src1 = ({ struct { uint64_t n : 32; } __x = { .n = src1 }; (uint64_t)__x.n; });src2 = ({ struct { uint64_t n : 32; } __x = { .n = src2 }; (uint64_t)__x.n; });word_t rs1 = src1;word_t rs2 = src2;word_t val=rs1/rs2;(cpu.gpr[check_reg_idx(dest)]) = val ; }; goto *(__instpat_end); } } while (0);
  do { uint64_t key, mask, shift; pattern_decode("0000001 ????? ????? 110 ????? 01100 11", (sizeof("0000001 ????? ????? 110 ????? 01100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); int64_t val=(int64_t)src1%(int64_t)src2;(cpu.gpr[check_reg_idx(dest)]) = val ; }; goto *(__instpat_end); } } while (0);
  do { uint64_t key, mask, shift; pattern_decode("0000001 ????? ????? 111 ????? 01100 11", (sizeof("0000001 ????? ????? 111 ????? 01100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); word_t val=src1%src2;(cpu.gpr[check_reg_idx(dest)]) = val ; }; goto *(__instpat_end); } } while (0);
  do { uint64_t key, mask, shift; pattern_decode("0000001 ????? ????? 110 ????? 01110 11", (sizeof("0000001 ????? ????? 110 ????? 01110 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); src1 = ({ struct { uint64_t n : 32; } __x = { .n = src1 }; (uint64_t)__x.n; });src2 = ({ struct { uint64_t n : 32; } __x = { .n = src2 }; (uint64_t)__x.n; });int32_t rs1 = src1;int32_t rs2 = src2;int32_t val=rs1%rs2;(cpu.gpr[check_reg_idx(dest)]) = ({ struct { int64_t n : 32; } __x = { .n = val }; (uint64_t)__x.n; }) ; }; goto *(__instpat_end); } } while (0);
  ...
  do { uint64_t key, mask, shift; pattern_decode("0011000 00010 00000 000 00000 11100 11", (sizeof("0011000 00010 00000 000 00000 11100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); s->dnpc = mret_inst(); ; }; goto *(__instpat_end); } } while (0);
  ...
  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 101 ????? 11000 11", (sizeof("??????? ????? ????? 101 ????? 11000 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_B); int rs1 = src1;int rs2 = src2;s->dnpc = (rs1>=rs2)?s->pc+imm:s->pc+0x4 ; }; goto *(__instpat_end); } } while (0);




  do { uint64_t key, mask, shift; pattern_decode("??????? ????? ????? 100 ????? 11000 11", (sizeof("??????? ????? ????? 100 ????? 11000 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_B); int rs1 = src1;int rs2 = src2;s->dnpc = ((rs1)<rs2)?s->pc+imm:s->pc+0x4 ; }; goto *(__instpat_end); } } while (0);
