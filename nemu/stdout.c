...
# 163 "src/isa/riscv64/inst.c"
  ...
  do { uint64_t key, mask, shift; pattern_decode("0011000 00010 00000 000 00000 11100 11", (sizeof("0011000 00010 00000 000 00000 11100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); s->dnpc = cpu.mepc; if(cpu.mstatus&(1<<7))cpu.mstatus = cpu.mstatus|(1<<3); else cpu.mstatus = cpu.mstatus&(~((1<<3))); cpu.mstatus = cpu.mstatus|(((1<<7))); cpu.mstatus = cpu.mstatus&0xFFFFFFFFFFFF7EFF; ; }; goto *(__instpat_end); } } while (0)
