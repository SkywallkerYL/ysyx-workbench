...
# 97 "src/isa/riscv64/inst.c"
  ...
  do { uint64_t key, mask, shift; pattern_decode("0000000 ????? ????? 011 ????? 01100 11", (sizeof("0000000 ????? ????? 011 ????? 01100 11") - 1), &key, &mask, &shift); if (((((s)->isa.inst.val) >> shift) & mask) == key) { { decode_operand(s, &dest, &src1, &src2, &imm, TYPE_RI); unsigned long immu = src2;(cpu.gpr[check_reg_idx(dest)]) = (src1<immu) ; }; goto *(__instpat_end); } } while (0);
