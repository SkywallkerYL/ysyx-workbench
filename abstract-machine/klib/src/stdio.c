...
int snprintf(char *out, size_t n, const char *fmt, ...) {
  ...
  va_start(ap,fmt);//初始化ap指针，其指向第一个可变参数
  //不能写到n， 最后要留一位+ \0;所以这里是<n-1
  int maxsize = n-1;
  bool tflag = 1;// \t一次+4,要特判以下
  while (*pstr!='\0' && resnum < maxsize &&tflag)
  {
    //switch里的break是跳出switch continue 是进入下一轮循环
    switch (*pstr)
    {
    case ' ':
      *out = *pstr;resnum++;break;
    case '\t':
      if((resnum+4)<maxsize) {*out = *pstr;resnum=resnum+4;break;}else{tflag = 0;break;}
    case '\n':
      *out = *pstr;resnum++;break;
    case '\r':
      *out = *pstr;resnum++;break;
    case '\%': // 进行格式解析
      ...
      case 's':
        ...
        //resnum += (unsigned int) lentemp;
        //printf ("%d\n",lentemp);
        for (char *p = strval; *p && resnum<maxsize; p++) {
          *out = *p;
          resnum++;
          out++;
          }
